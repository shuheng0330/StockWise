...
## Main Features
- KPI summary cards
- AI Decision Brief with asynchronous loading, fallback/safety state, impact summary, trade-offs, and recommended action order
- AI Advisor panel with starter prompts, free-text input, and structured answer cards
...
- From simulation handoff, preload the AI Advisor with the simulated item context and seeded prompt
...
- Hide placeholder warning values such as `none`, `null`, `n/a`, or empty text so the yellow warning callout appears only for meaningful warnings.

## AI Decision Brief

## Purpose
Show the owner a dashboard-level operating plan generated by Z.AI GLM from the current analysis, without blocking the normal dashboard load.

## Backend Endpoint
- `GET /api/v1/analyses/{analysis_id}/decision-brief`

## Loading Behavior
- The dashboard first loads `GET /api/v1/analyses/{analysis_id}` and renders KPI cards, filters, AI Advisor, and the item table.
- The decision brief request starts in parallel and uses its own loading state.
- If the brief fails or falls back, the dashboard remains usable and the item table stays visible.

## Main Features
- Header with source badge: `live`, `mock`, or `fallback`
- Safety badge: `validated`, `retried`, or `fallback_used`
- Summary of the recommended operating plan
- Action groups:
  - `buy_today[]`
  - `buy_less[]`
  - `delay[]`
...
- `confidence_note`
- optional `warning_flag`
- Fallback state with a `Review Records` path for human review before ordering
- Confidence note should explicitly reinforce when the brief is based on history plus current records, for example deterministic daily usage rates and lead times over the source record count.
...
- Decision brief loading
- Decision brief fallback or failure used
